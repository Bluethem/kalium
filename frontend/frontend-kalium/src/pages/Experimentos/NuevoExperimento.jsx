...
const NuevoExperimento = () => {
  ...
  const [experimentoCreado, setExperimentoCreado] = useState(null);
  ...
  const cargarTiposInsumo = async () => {
    try {
      ...
      setErrorMessage('Error al cargar los tipos de insumo');
      setShowError(true);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log('Creando experimento:', experimentoData);
      const expResponse = await experimentoService.createExperimento(experimentoData);
      const experimentoId = expResponse.data.idExperimento;
      console.log('Experimento creado con ID:', experimentoId);

      // 2. Agregar los detalles (insumos)
      for (const insumo of insumosSeleccionados) {
        const detalleData = {
          ...
          experimento: { idExperimento: experimentoId }
        };
      ...
      setExperimentoCreado(expResponse.data);
      setShowSuccess(true);
    } catch (error) {
      console.error('Error completo:', error);
      console.error('Respuesta del error:', error.response);
      setErrorMessage(error.response?.data || error.message || 'No se pudo crear el experimento');
      setShowError(true);
    } finally {
      setLoading(false);
    }
    ...
    <div className="flex flex-col min-h-screen bg-[#f6f6f8] dark:bg-[#111621]">
      ...
      <main className="flex-1 overflow-y-auto p-6 lg:p-8">
        <div className="mx-auto max-w-4xl">
          <div className="mb-8">
            <h2 className="text-3xl font-bold tracking-tight text-gray-900 dark:text-white">Nuevo Experimento</h2>
            <p className="text-gray-500 dark:text-gray-400">Complete el formulario para crear un nuevo experimento.</p>
          </div>
          ...
          <div className="w-full max-w-md rounded-xl bg-white dark:bg-gray-900 p-8 shadow-2xl border border-gray-200 dark:border-gray-800">
            <div className="flex flex-col items-center text-center">
              ...
              <p className="text-gray-600 dark:text-gray-400 mb-8">
                El experimento "{experimentoCreado?.nombreExperimento}" ha sido creado correctamente.
              </p>
              <div className="flex w-full flex-col gap-3 sm:flex-row sm:justify-center">
                <button
                  onClick={() => navigate(`/experimentos/${experimentoCreado?.idExperimento}`)}
