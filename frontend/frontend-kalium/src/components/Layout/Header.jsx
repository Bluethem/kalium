import React from 'react';
import { Link, useLocation } from 'react-router-dom';
import { Bell } from 'lucide-react';

const Header = () => {
  const location = useLocation();

  const isActive = (path) => {
    return location.pathname === path || location.pathname.startsWith(path + '/');
  };

  return (
    <header className="flex items-center justify-between whitespace-nowrap border-b border-gray-200 dark:border-gray-800 px-10 py-3 bg-white dark:bg-gray-900">
      <div className="flex items-center gap-4 text-gray-900 dark:text-white">
        <div className="size-6 text-[#14378f]">
          <svg fill="none" viewBox="0 0 48 48" xmlns="http://www.w3.org/2000/svg">
            <path
              clipRule="evenodd"
              d="M24 18.4228L42 11.475V34.3663C42 34.7796 41.7457 35.1504 41.3601 35.2992L24 42V18.4228Z"
              fill="currentColor"
              fillRule="evenodd"
            />
            <path
              clipRule="evenodd"
              d="M24 8.18819L33.4123 11.574L24 15.2071L14.5877 11.574L24 8.18819ZM9 15.8487L21 20.4805V37.6263L9 32.9945V15.8487ZM27 37.6263V20.4805L39 15.8487V32.9945L27 37.6263ZM25.354 2.29885C24.4788 1.98402 23.5212 1.98402 22.646 2.29885L4.98454 8.65208C3.7939 9.08038 3 10.2097 3 11.475V34.3663C3 36.0196 4.01719 37.5026 5.55962 38.098L22.9197 44.7987C23.6149 45.0671 24.3851 45.0671 25.0803 44.7987L42.4404 38.098C43.9828 37.5026 45 36.0196 45 34.3663V11.475C45 10.2097 44.2061 9.08038 43.0155 8.65208L25.354 2.29885Z"
              fill="currentColor"
              fillRule="evenodd"
            />
          </svg>
        </div>
        <h1 className="text-xl font-bold">Kalium</h1>
      </div>

      <nav className="flex flex-1 justify-center items-center gap-8 text-sm font-medium text-gray-600 dark:text-gray-300">
        <Link
          to="/"
          className={`hover:text-[#14378f] dark:hover:text-[#14378f] transition-colors ${
            isActive('/') && location.pathname === '/' ? 'text-[#14378f] font-bold' : ''
          }`}
        >
          Dashboard
        </Link>
        <Link
          to="/insumos"
          className={`hover:text-[#14378f] dark:hover:text-[#14378f] transition-colors ${
            isActive('/insumos') ? 'text-[#14378f] font-bold' : ''
          }`}
        >
          Insumos
        </Link>
        <Link
          to="/pedidos"
          className={`hover:text-[#14378f] dark:hover:text-[#14378f] transition-colors ${
            isActive('/pedidos') ? 'text-[#14378f] font-bold' : ''
          }`}
        >
          Pedidos
        </Link>
        <Link
          to="/reportes"
          className={`hover:text-[#14378f] dark:hover:text-[#14378f] transition-colors ${
            isActive('/reportes') ? 'text-[#14378f] font-bold' : ''
          }`}
        >
          Reportes
        </Link>
      </nav>

      <div className="flex items-center gap-4">
        <button className="p-2 rounded-full hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-600 dark:text-gray-300 transition-colors">
          <Bell size={20} />
        </button>
        <div
          className="size-10 rounded-full bg-cover bg-center bg-gray-300 dark:bg-gray-600"
          style={{
            backgroundImage:
              'url("https://ui-avatars.com/api/?name=Usuario&background=14378f&color=fff")',
          }}
        />
      </div>
    </header>
  );
};

export default Header;
